...
// Run: php -d short_open_tag=1 convert_phpspreadsheet.php
...
foreach($dump["fil_org"] As $key=>$fil)
{
    ...
    $bld_purpose_id = array_key_exists($dump['bld_purpose_x'], $key) ? $dump['bld_purpose_x'][$key] : -1;
    $bld_purpose = array_key_exists($bld_purpose_id, $dump['bld_purpose']) ? $dump['bld_purpose'][$bld_purpose_id] : "";

    $bld_name_id = array_key_exists($dump['bld_name_x'], $key) ? $dump['bld_name_x'][$key] : -1;
